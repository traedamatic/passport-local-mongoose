...
module.exports = function(schema, options) {
    ...
    schema.statics.verifyAccessToken = function (accessToken, cb) {
        ...
        try {
            ...
            //check cache and then return if the cache holds the accessToken
            var decodedAccessTokenFromCache = accessTokenCache.get(accessToken);
            if (decodedAccessTokenFromCache != undefined) {
                return decodedAccessTokenFromCache;
            }
            ...
            self.findOne({"_id": decodedAccessToken._id}, function (err, user) {
                ...
                accessTokenCache.set(accessToken, user.toString(), 3600);
     ...
     */
    schema.statics.clearAccessTokenCache = function () {
        accessTokenCache.del(accessTokenCache.keys());
    };
     ...
     * @returns {boolean}
     */
    schema.statics.deleteAccessTokenFromCache = function (key) {

        if (!key) {
            return false;
        }

        accessTokenCache.del(key);
    };
